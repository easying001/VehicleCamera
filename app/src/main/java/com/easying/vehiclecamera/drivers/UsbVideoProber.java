...
import com.easying.vehiclecamera.drivers.UsbDriver.SonixUsbStorageDriver;
import com.easying.vehiclecamera.drivers.UsbDriver.SonixUsbVideoDriver;
...
public class UsbVideoProber {
    ...
    public void addUsbDriver(UsbDevice device) {
        ...
        if (driver != null) {
            ...
            LogUtils.tag("Driver-Prober").d("No suitable driver found for this device");
        }
        ...
        if (isDriverLoaded(device)) {
            for(IUsbDriver driver : mUsbDriverList) {
                if (driver.getDevice().getProductId() == device.getProductId()
                    ...
                    LogUtils.tag("Driver-Prober").d("Remove driver for this device");
                }
            }
        }
    }
    ...
    public boolean isDriverLoaded(UsbDevice device) {
        ...
        if ((mUsbDriverList != null) && (mUsbDriverList.size() > 0)) {
            for(IUsbDriver driver : mUsbDriverList) {
                if (driver.getDevice().getProductId() == device.getProductId()
                        && (driver.getDevice().getDeviceId() == device.getDeviceId())) {
                    LogUtils.tag("Driver-Prober").d("Driver for this device has been loaded");
                    return true;
                }
            }
        }
