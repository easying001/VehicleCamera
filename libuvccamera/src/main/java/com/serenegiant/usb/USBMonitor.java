...
/*		if (listener == null)
	...
	public synchronized void register() {
		if (mPermissionIntent == null) {
			...
			mDeviceCounts = 0;
			mHandler.postDelayed(mDeviceCheckRunnable, 2000);
		}
	}
	 ...
	 * @param device
	 */
	public synchronized void requestPermission(final UsbDevice device, UsbDeviceConnection connection) {
		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
		if (mPermissionIntent != null) {
			if (device != null) {
				if (mUsbManager.hasPermission(device)) {
					processConnect(device, connection);
				} else {
	...
	private final void processConnect(final UsbDevice device, final UsbDeviceConnection connection) {
		...
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				if (ctrlBlock == null) {
					ctrlBlock = new UsbControlBlock(USBMonitor.this, device, connection);
	...
	public static final class UsbControlBlock {
		 ...
		 * @param device
		 */
		public UsbControlBlock(final USBMonitor monitor, final UsbDevice device, UsbDeviceConnection connection) {
